...
/**
 * Mark attendance notifications as viewed for a staff member
 * Updates notification_viewed to true for approved/rejected attendance records
 */
export async function markAttendanceAsViewed(staffId: string): Promise<void> {
  ...
  if (error) {
    console.error('Failed to mark attendance as viewed:', error.message);
    throw new Error(`Failed to mark attendance notifications as viewed: ${error.message}`);
  }
}
