 ...
 */
export async function clearConversation(user1Id: string, user2Id: string): Promise<void> {
  const result = await supabaseService.deleteConversation(user1Id, user2Id);
  if (!result.success) {
    throw new Error(result.error.message);
  }
}
